...
begin
  if internet=nil then
    internet:=TWinInternet.Create('Billy Engine microtransaction system');

  ss:=tstringstream.create;
  try
    try
      internet.getURL('https://billyengine.org/microtransaction.php?action=buy&amount='+inttostr(tbitbtn(sender).Tag), ss);
      ...
      MessageDlg('Sorry, but the Billy Engine server is currently overloaded or you have no internet. Please try again later',mtInformation, [mbok],0);
    end;
  finally
    ss.free;
  end;
