...
begin
  ...
  if launch32bit then
  begin
    if fileexists(selfpath+'billyengine-i386.exe') then
      ShellExecute(0, 'open', pchar(selfpath+'billyengine-i386.exe'), pchar(param), pchar(selfpath), sw_show)
    else
      MessageBox(0, 'billyengine-i386.exe could not be found. Please disable/uninstall your anti virus and reinstall Billy Engine to fix this','Billy Engine launch error',MB_OK or MB_ICONERROR);
  end
  else
  begin
    if FileExists(selfpath+'billyengine-x86_64.exe') then
      ShellExecute(0, 'open', pchar(selfpath+'billyengine-x86_64.exe'), pchar(param), pchar(selfpath), sw_show)
    else
      MessageBox(0, 'billyengine-x86_64.exe could not be found. Please disable/uninstall your anti virus and reinstall Billy Engine to fix this','Billy Engine launch error',MB_OK or MB_ICONERROR);
  end;
