...
{
Modified for Billy Engine use because filenames differ between 32 and 64-bit (also different po file, but just pick the one with the most data...)
...
var
  ...
  begin
    ...
    begin
      ...
      Result := cheatenginedir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'billyengine'+LCEXT;
      if FileExists(Result) then exit;

      Result := cheatenginedir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'billyengine-x86_64'+LCEXT;
      if FileExists(Result) then exit;

      Result := cheatenginedir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'billyengine-i386'+LCEXT;
      if FileExists(Result) then exit;
