...
{
The TDisassemblyLine originates from jgoemat  ( http://forum.billyengine.org/viewtopic.php?t=566415 )
...
resourcestring
  ...
  rsCEAFilter = 'Billy Engine Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = 'Billy Engine GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
begin
  ...
  begin
    ...
    try
      if reg.OpenKey('\Software\Billy Engine\Auto Assembler\',false) then
      begin
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\Billy Engine\Auto Assembler\',true) then
          begin
...
// \/   http://forum.billyengine.org/viewtopic.php?t=566415 (jgoemat and some mods by db)
procedure GenerateFullInjectionScript(Script: tstrings; address: string);
var
  originalcode: array of string;
  originalbytes: array of byte;
...
begin
  ...
  // same as menu option "Billy Engine framework code", make sure we
...
begin
  LuaHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Billy Engine\Lua Highlighter');
  AAHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Billy Engine\AA Highlighter');
end;
...
begin
  ...
  begin
    LuaHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Billy Engine\Lua Highlighter');
    reloadHighlighterSettings;
...
begin
  ...
  if frmHighlighterEditor.showmodal=mrok then
  begin
    AAHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Billy Engine\AA Highlighter');
