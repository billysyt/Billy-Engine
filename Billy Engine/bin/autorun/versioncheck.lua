--version check update script for Billy Engine
...
function CheckVersion(automatic)
  ...
  if versionCheckThread==nil then
    versionCheckThread=createThread(function(t)
        ...
        local r=i.getURL('https://billyengine.org/latestversion.txt')
        ...
        if r then
          ...
          t.synchronize(function()
            if newerVersion then
              if messageDialog(string.format(translate('Billy Engine %s is available at www.billyengine.org. Go there now?'),latestVersionString), mtConfirmation, mbYes, mbNo)==mrYes then
                shellExecute('https://billyengine.org/?versioncheck=1')
              else
...
cbCheckForUpdatesOnLaunch.Caption=translate('Check for updates when Billy Engine starts')..'.'
