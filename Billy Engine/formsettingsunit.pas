...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in Billy Engine finding NO memory! Are you stupid?';
  ...
  rsPopupHideCheatEngine = 'Popup/Hide Billy Engine';
  ...
  rsRestartCE = 'It is recommended to restart Billy Engine for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.WriteBinaryData('Show Billy Engine Hotkey',frameHotkeyConfig.newhotkeys[1][0],10);
        ...
        begin
          if reg.OpenKey('\Software\Billy Engine\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\Billy Engine\Tools');
      if Reg.OpenKey('\Software\Billy Engine\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\Billy Engine\Plugins'+cpu);
      if Reg.OpenKey('\Software\Billy Engine\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    n:=f[i];
    if not (fileexists(n+'\billyengine.po') or fileexists(n+'\billyengine-x86_64.po') or fileexists(n+'\billyengine-i386.po')) then
      continue;
