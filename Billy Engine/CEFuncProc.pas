...
//Billy Engine regular WONT look at this
...
//http://forum.billyengine.org/viewtopic.php?t=564382
type
...
begin
  ...
  try
    ...
    try
      ...
      if Reg.OpenKey('\Software\Billy Engine',false) then
      begin
        if reg.valueexists('Save window positions') then
          if reg.readbool('Save window positions') = false then exit;
      end;

      if Reg.OpenKey('\Software\Billy Engine\Window Positions '+inttostr(screen.PixelsPerInch),false) or Reg.OpenKey('\Software\Billy Engine\Window Positions',false) then
      begin
        s:=form.Name;
        s:=s+rsPosition;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Billy Engine\Window Positions '+inttostr(screen.PixelsPerInch),true) then
      begin
...
begin
  ...
  path:=path+'Billy Engine Symbols';
