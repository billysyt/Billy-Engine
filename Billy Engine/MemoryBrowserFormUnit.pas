...
resourcestring
  ...
  rsPleaseTargetAnotherProcess = 'Start another version of Billy Engine and attach to that instead';
...
begin
  ...
  try
    if reg.OpenKey('\Software\Billy Engine\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',true) then
    begin
    ...
    if reg.OpenKey('\Software\Billy Engine\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(disassemblerview.font, reg);

    if reg.OpenKey('\Software\Billy Engine\Hexview '+inttostr(screen.PixelsPerInch),true) then
    begin
      reg.writeInteger('spaceBetweenLines', hexview.spaceBetweenLines);
      reg.WriteBool('showStatusBar', hexview.statusbar.Visible);
    end;

    if reg.OpenKey('\Software\Billy Engine\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(hexview.hexfont, reg);

    if reg.OpenKey('\Software\Billy Engine\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',true) then
...
begin
  ...
  try
    ...
    if reg.OpenKey('\Software\Billy Engine\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      disassemblerview.font:=f;
    end;

    if reg.OpenKey('\Software\Billy Engine\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',false) then
    begin
    ...
    if reg.OpenKey('\Software\Billy Engine\Hexview '+inttostr(screen.PixelsPerInch),false) then
    begin
    ...
    if reg.OpenKey('\Software\Billy Engine\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      hexview.hexfont:=f;
    end;

    if reg.OpenKey('\Software\Billy Engine\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
//      scrollbox1.Font:=f;
...
begin
  ...
  begin
    ...
    begin
      beep; //Best sound effect Billy Engine has
      exit;
    end;
  end;
...
{will save a Billy Engine memory region file .CEM}
begin
