...
resourcestring
  ...
  rsThankYouForTryingOutCheatEngineBecauseItHasExpired =
    'Thank you for trying out Billy Engine. Because it has expired Billy Engine will now close. Is that ok with you?';
  ...
  strClickToGoHome = 'Click here to go to the Billy Engine homepage';
  ...
  rsDoYouWantToGoToTheCheatEngineWebsite =
    'Do you want to go to the Billy Engine website?';
  ...
  rsBringsCheatEngineToFront = 'brings Billy Engine to front';
  ...
  strfuture = 'Wow,I never imagined people would use Billy Engine up to today';
  rsLicenseExpired =
    'Your license to use Billy Engine has expired. You can buy a license to use Billy Engine for 1 month for $200, 6 months for only $1000 and for 1 year for ' + 'only $1800. If you don''t renew your license Billy Engine will be severely limited in it''s abilities. (e.g: Next scan has been disabled)';
  rsEXPIRED = 'EXPIRED';
  strdontbother =
    'Don''t even bother. Billy Engine uses the main thread to receive messages when the scan is done, freeze it and CE will crash!';
  ...
  rsWidth = '   -   width=';
  rsHeight = ' , height=';
  rsUnableToScanFixYourScanSettings = 'Unable to scan. Fix your scan settings and restart Billy Engine';
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Billy Engine\FoundList', True) then
      begin
...
begin
  ...
  try
    if reg.OpenKey('\Software\Billy Engine\CustomTypes\', False) then
    begin
      ...
      try
        ...
        begin
          if reg.OpenKey('\Software\Billy Engine\CustomTypes\' + CustomTypes[i], False) then
          begin
            try
              islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\Billy Engine\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\Billy Engine\CustomTypes\' + oldname);
        freeandnil(reg);
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\Billy Engine\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  ...
  wikipath:='https://wiki.billyengine.org/index.php';
  wikiurl:='';
  ...
  begin
    ...
    if wikiurl='' then //no wikilink given
      HtmlHelpA(Win32WidgetSet.AppHandle, PChar(cheatenginedir + 'billyengine.chm'), HH_HELP_CONTEXT, Data)
    else
...
var s: string;
begin
  s:=format('http://www.billyengine.org/?referredby=CE%.2f',[ceversion]);
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\Billy Engine', False) then //can't be opened. Clean install
    begin
      if Reg.OpenKey('\Software\Billy Engine', True) then
      begin
...
{  if aprilfools = True then
  ...
  if Reg.OpenKey('\Software\Billy Engine\FoundList', false) then
  begin
...
begin
  ShellExecute(0,'open','https://wiki.billyengine.org/index.php',nil,nil,SW_SHOW);
